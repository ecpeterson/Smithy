...
let select_line_loop x y map =
    ...
    let build_array () =
        ...
        let test_line n =
            ...
            let (p0x, p0y) = (Array.get points p0)#vertex () in
            let (p1x, p1y) = (Array.get points p1)#vertex () in
        ...
        let rec fill_array arr lines_position array_position =
            ...
            if test_line lines_position then begin
                Array.set arr array_position lines_position;
        ...
        let sort_helper l1 l2 =
            let l1 = get_intersection_point (Array.get lines l1) in
            let l2 = get_intersection_point (Array.get lines l2) in
    ...
    let rec get_neighbors index p =
        ...
        let (p0, p1) = (Array.get lines index)#endpoints () in
    ...
    let rec build_loop target prev working rec_depth starter =
        let prev_vtx = (Array.get points prev)#vertex () in
        let working_vtx = (Array.get points working)#vertex () in
        ...
        let rec get_first_neighbor lst =
            ...
            let line = Array.get lines line_idx in
            let (p0, p1) = line#endpoints () in
            let p0_vtx = (Array.get points p0)#vertex () in
            let p1_vtx = (Array.get points p1)#vertex () in
        ...
        let (p0, p1) = (Array.get lines tightest)#endpoints () in
    ...
    let rec array_iter n starters =
        ...
        let starter = Array.get starters n in
        let (ep0, ep1) = (Array.get lines starter)#endpoints () in
        let (_, p0y) = (Array.get points ep0)#vertex () in
        let (_, p1y) = (Array.get points ep1)#vertex () in
...
let fill_poly x y map =
    ...
    match List.fold_left (fun x y -> x && (
        let line = (Array.get (map#get_lines_array ()) y) in
    ...
    let rec build_point_loop line_loop point_loop =
        match (line_loop, point_loop) with
            (line :: lines, point :: points) ->
                let (p0, p1) = (Array.get (map#get_lines_array()) line)#endpoints () in
                build_point_loop lines ((if p0 = point then p1 else p0) :: point_loop)
            |([], _) -> point_loop in
    let line1 :: line2 :: remaining_lines = line_loop in
    let (p0, p1) = (Array.get (map#get_lines_array ()) line1)#endpoints () in
    ...
    List.iter (fun x ->
        let line = Array.get (map#get_lines_array ()) x in
...
let start_line x y map choose_distance =
    ...
    if point_distance < choose_distance && nearest_point >= 0 then
        ...
        let line = Array.get (map#get_lines_array ()) nearest_line in
...
let draw_line x y map gl =
    ...
    let (s, t) = (Array.get (map#get_points_array ()) !start_point)#vertex () in
...
let connect_line x y map gl choose_distance =
    let do_line target_point =
        let (p0x, p0y) = (Array.get (map#get_points_array ()) !start_point)#vertex () in
        let (p1x, p1y) = (Array.get (map#get_points_array ()) target_point)#vertex () in
